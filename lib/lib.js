...
module.exports.SERVER_SIGNATURE = generateServerSignature();

module.exports.hmac = function(data, key){
    var hmac = crypto.createHmac("SHA256", key || require("../config").server_key);
    hmac.update(data);
    return hmac.digest("hex");
};

module.exports.prettyPrint = function(str){
    ...
    console.log(Array(max+1 + 4).join("-"));
    console.log("|" + Array(max+1 + 2).join(" ") + "|")
    
    for(var i=0, len = str.length; i<len; i++){
        if(str[i].length >= 3 && str[i] == Array(str[i].length+1).join(str[i].substr(0,1))){
            console.log("| "+Array(max+1).join(str[i].substr(0,1))+" |")
        }else{
            console.log("| " + str[i] + (str[i].length<max?Array((max - str[i].length)+1).join(" "):"") + " |");
        }
    }
...
module.exports.HTMLEntitiesReversed = (function(module){
    entities = module.HTMLEntities || {};
    var keys = Object.keys(entities),
        reversed = {}, ent;
    module.HTMLEntityStr = "";
    for(var i=0, len=keys.length; i<len; i++){
        reversed[entities[keys[i]]] = keys[i];
        ent = entities[keys[i]].toString(16);
        for(var j=0, jlen = 4 - ent.length; j<jlen; j++)ent = "0"+ent;
        module.HTMLEntityStr += "\\u" + ent;
    }
    return reversed;
})(module.exports);

module.exports.HTMLEntityRegex = new RegExp(module.HTMLEntityStr,"g");


module.exports.HTMLSpecialChars = function(str){
    str = (str || "").toString("utf-8").trim();
    return str.replace(lib.HTMLEntityRegex, function(chr){
        return lib.HTMLEntitiesReversed[chr.charCodeAt(0)];
    });
}

module.exports.resolvePath = function(path, base){

    var base_path, target_path, final_path;
    ...
    if(final_path.length && (path || "/").substr(-1)=="/"){
        final_path.push("");
    }
...
module.exports.detectMimeType = function(filename){
    var ext = (filename || "").split(".").pop().toLowerCase().trim(),
        mime = mimelib.contentTypes[ext] || "application/octet-stream";
...
function generateServerSignature(){
    var list = [], time;
    try{
        time = fs.statSync(__filename).mtime.getTime();
    }catch(E){}
    if(!time){
        time = Date.now();
    }
    try{
        list = fs.readFileSync(__dirname + "/../data/server_signatures.txt", "utf-8").trim().split("\n");
    }catch(E){}
    if(!list.length){
        return "Apache";
        return;
